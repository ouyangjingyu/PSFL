...
def TierScheduler(client_allocation):
    """
    Assigns tier levels (1-7) to clients based on their device resource value.
    
    Args:
        clients_store (dict): Dictionary mapping client indices to their resource value (float)
        
    Returns:
        client_tier (list): List of tier assignments (1-7) for each client
        computation_time_clients (float): Time taken to execute tierscheduler
    """
    start_time = time.time()

    client_tier = []
    client_num_tier = [0] * 7  # Track number of clients in each tier (1-7)
    num_clients = len(client_allocation)
    # Assign tiers based on storage_tier from client_resource_allocation
    for i in range(num_clients):
        tier = client_allocation[i]["storage_tier"]  # Get tier (1-7) directly from allocation
        client_tier.append(tier)
        client_num_tier[tier-1] += 1  # Increment count for this tier

    # Convert to numpy array for easier manipulation later
    client_tier = np.array(client_tier) 
    client_number_tier = np.array(client_num_tier)

    tierschedule_time_clients = time.time() - start_time

    return client_tier,client_number_tier,tierschedule_time_clients
