...
class TierHFLServerGroup:
    ...
    def update_feature_cache(self, client_id, features, labels):
        """更新特征缓存"""
        self.feature_cache[client_id] = {
            'features': features.detach().cpu(),
            'labels': labels.detach().cpu(),
            'timestamp': time.time()
        }
    
    def get_cached_features(self, max_age=300):
        """获取缓存的特征，不超过指定的最大时间"""
        current_time = time.time()
        features = []
        labels = []
        
        for client_id, cache in self.feature_cache.items():
            if current_time - cache['timestamp'] <= max_age:
                features.append(cache['features'])
                labels.append(cache['labels'])
