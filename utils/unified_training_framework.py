...
def train_client_with_unified_server(client_id, client_model, unified_server_model, 
                                    ...
                                    local_epochs=1, feature_monitor=None, **kwargs):
    """
    在统一服务器模型架构下的客户端训练函数
    ...
    try:
        ...
        for epoch in range(local_epochs):
            ...
            for batch_idx, (data, target) in enumerate(client.train_data):
                ...
                # 8. 总损失 - 平衡本地和全局损失
                total_loss = 0.5 * local_loss + 0.5 * global_loss
                ...
                torch.nn.utils.clip_grad_norm_(unified_server_model.parameters(), max_norm=5.0)
                torch.nn.utils.clip_grad_norm_(global_classifier.parameters(), max_norm=5.0)
