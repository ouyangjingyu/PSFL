...
from utils.fedavg import enhanced_aggregated_fedavg
...
# 导入模型诊断工具和聚合改进代码
from utils.model_diagnosis import *
from utils.aggregation_fixes import *
...
#=====================================================================================================        
def calibrate_global_bn(model, dataloader, device, num_batches=10):
    """使用代表性数据集专门校准全局模型的BN层"""
    print("\n校准全局模型BatchNorm层...")
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.BatchNorm1d) or isinstance(module, nn.BatchNorm2d):
            module.reset_running_stats()  # 重置BN统计量
            module.momentum = 0.1  # 设置较小的动量参数加快更新
        ...
        for name, module in model.named_modules():
            if isinstance(module, nn.BatchNorm1d) or isinstance(module, nn.BatchNorm2d):
                if hasattr(module, 'running_var'):
                    module.running_var.clamp_(min=0.01, max=3.0)
                if hasattr(module, 'running_mean'):
                    module.running_mean.clamp_(min=-1.0, max=1.0)
    
    print("BatchNorm层校准完成")
    return model
# 替换原有的evaluate_global_model_on_all_test函数
def enhanced_evaluate_global_model_on_all_test(net_glob, dataset_test, device, client_number, ell):
    """
    在所有客户端的测试集上评估全局模型，带详细诊断
    """
    # 先确保模型在正确的设备上
    net_glob.to(device)
    ...
    # 先进行批标准化层参数检查
    print("\n检查全局模型批标准化层参数...")
    bn_stats, bn_issues = analyze_batchnorm_layers(net_glob)
    
    if bn_issues:
        print(f"全局模型存在 {len(bn_issues)} 个批标准化层问题:")
        for issue in bn_issues[:5]:  # 只显示前5个问题
            print(f"  - {issue}")
        
        print("正在修复批标准化层问题...")
        normalize_bn_for_inference(net_glob)
        
        # 检查修复后的状态
        bn_stats_after, bn_issues_after = analyze_batchnorm_layers(net_glob)
        if bn_issues_after:
            print(f"修复后仍存在 {len(bn_issues_after)} 个问题")
        else:
            print("批标准化层问题已修复")
    ...
    # 检测不平衡预测
    max_percent = dist_percent.max()
    min_percent = dist_percent.min()
    expected_percent = 10.0  # 假设10个类别，每类期望10%
    
    if max_percent > expected_percent * 2 or min_percent < expected_percent * 0.5:
        print("\n警告: 检测到严重的类别不平衡预测")
        print(f"  预期每类占比: {expected_percent:.1f}%")
        print(f"  最低类别占比: {min_percent:.2f}%")
        print(f"  最高类别占比: {max_percent:.2f}%")
        
        # 如果某一类别占比超过50%，建议重置分类器
        if max_percent > 50.0:
            print("建议: 应考虑重置分类器权重")
    ...
    # 添加类别分布记录
    for i, percent in enumerate(dist_percent):
        wandb.log({f"Class_{i}_Prediction_Percent": percent, "epoch": ell}, commit=False)
    
    # 计算和记录类别不平衡指标
    imbalance_ratio = max_percent / (min_percent if min_percent > 0 else 0.1)
    wandb.log({"Prediction_Imbalance_Ratio": imbalance_ratio, "epoch": ell}, commit=False)
    ...
    try:
        ...
        cm = confusion_matrix(true_labels_all, predicted_labels_all, labels=range(10))
        plt.figure(figsize=(10, 8))
        sns.heatmap(cm, annot=True, fmt='d', cmap='Blues')
        plt.title(f'Confusion Matrix (Epoch {ell})')
        plt.ylabel('True Label')
        plt.xlabel('Predicted Label')
        plt.xticks(np.arange(10) + 0.5, range(10))
        plt.yticks(np.arange(10) + 0.5, range(10))
        plt.savefig(f'confusion_matrix_epoch_{ell}.png')
        plt.close()
    except Exception as e:
        print(f"生成混淆矩阵时出错: {str(e)}")
    
    return global_accuracy, client_metrics



# 更新diagnose_and_fix_aggregated_model函数，确保模型在正确设备上
def diagnose_and_fix_aggregated_model(net_glob_fed, tier_models, device, test_data):
    """
    诊断和修复聚合后的全局模型
    """
    print("\n开始诊断和修复聚合后的模型...")
    ...
    # 确保模型在正确设备上
    net_glob_fed = net_glob_fed.to(device)
    
    # 步骤1: 全面诊断模型
    try:
        diagnostic_results = full_model_diagnostic(
            net_glob_fed, 
            test_data, 
            device,
            num_classes=10,
        ...
        print(f"模型诊断出错: {str(e)}")
        print("尝试修复模型设备问题并再次诊断...")
        
        # 确保模型完全在正确设备上
        net_glob_fed = net_glob_fed.to(device)
        for param in net_glob_fed.parameters():
    ...
    # 步骤2: 检查是否需要修复批标准化层
    bn_issues = diagnostic_results['bn_issues']
    dead_neurons = diagnostic_results['dead_neurons']
    classifier_issues = diagnostic_results['classifier_issues']
    
    needs_bn_fix = len(bn_issues) > 0
    needs_activation_fix = len(dead_neurons) > 0
    needs_classifier_fix = len(classifier_issues) > 0
    
    # 步骤3: 执行必要的修复
    if needs_bn_fix:
        print("\n修复批标准化层问题...")
        # 从健康的tier模型中选择BN统计量
        healthy_bn_found = False
        
        for tier in range(1, len(tier_models) + 1):
            if tier in tier_models:
                tier_bn_stats, tier_bn_issues = analyze_batchnorm_layers(tier_models[tier])
                if len(tier_bn_issues) == 0:
                    print(f"使用Tier {tier}的健康BN统计量")
                    # 复制健康模型的BN统计量
                    copy_batchnorm_stats(tier_models[tier], net_glob_fed)
                    healthy_bn_found = True
                    break
        
        if not healthy_bn_found:
            print("未找到健康的BN统计量，进行重置...")
            fix_batchnorm_statistics(net_glob_fed, reset_running_stats=True)
    
    # 步骤4: 校准BatchNorm层
    if needs_bn_fix or needs_activation_fix:
        print("\n使用测试数据校准模型...")
        net_glob_fed = calibrate_model_with_data(net_glob_fed, test_data, device, num_batches=20)
    
    # 步骤5: 修复分类器问题
    if needs_classifier_fix:
        print("\n检测到分类器问题，考虑重置分类器权重...")
        
        # 获取预测分布
        pred_data = diagnostic_results['prediction_data']
        pred_counts = pred_data['prediction_counts']
        total_preds = sum(pred_counts)
        pred_percentages = [count/total_preds*100 for count in pred_counts]
        
        # 检查是否存在严重不平衡
        max_percent = max(pred_percentages)
        min_percent = min(pred_percentages)
        
        if max_percent > 30 or min_percent < 5:
            print(f"检测到严重不平衡: 最高{max_percent:.1f}%, 最低{min_percent:.1f}%")
            print("重置分类器权重...")
            net_glob_fed = reset_classifier_weights(net_glob_fed, num_classes=10)
    
    # 最终验证修复结果
    net_glob_fed.eval()
    final_diagnostic = full_model_diagnostic(
        net_glob_fed,
        test_data,
        device,
        num_classes=10,
    ...
    print("\n修复后的模型状态:")
    bn_issues_after = final_diagnostic['bn_issues']
    dead_neurons_after = final_diagnostic['dead_neurons']
    
    if len(bn_issues_after) == 0 and len(dead_neurons_after) == 0:
        print("模型已成功修复!")
    else:
        print(f"模型仍有 {len(bn_issues_after)} 个BN问题和 {len(dead_neurons_after)} 个死亡神经元")
...
def copy_batchnorm_stats(source_model, target_model):
    """
    从源模型复制批标准化层的统计量到目标模型
    """
    source_state = source_model.state_dict()
    target_state = target_model.state_dict()
    
    # 复制BN统计量
    bn_keys = [k for k in source_state.keys() if 'running_mean' in k or 'running_var' in k]
    
    for k in bn_keys:
        if k in target_state:
            target_state[k] = source_state[k].clone()
    
    # 加载更新后的状态字典
    target_model.load_state_dict(target_state)

def debug_feature_distribution(model, dataloader, device, num_batches=5, layer_name=None):
    """
    调试指定层的特征分布
    """
    print(f"\n分析模型特征分布...")
    ...
    with torch.no_grad():
        for i, (images, _) in enumerate(dataloader):
            ...
            for name, act in activation.items():
                # 转换为一维数组进行分析
                act_flat = act.view(-1).cpu().numpy()
                
                # 计算基本统计数据
                act_mean = act_flat.mean()
                act_std = act_flat.std()
                act_min = act_flat.min()
                act_max = act_flat.max()
                
                # 计算零值百分比
                zero_percent = (act_flat == 0).mean() * 100
                
                print(f"\n层 {name} 激活统计:")
                print(f"  均值: {act_mean:.6f}")
                print(f"  标准差: {act_std:.6f}")
                print(f"  范围: [{act_min:.6f}, {act_max:.6f}]")
                print(f"  零值百分比: {zero_percent:.2f}%")
                
                # 检测异常
                if zero_percent > 50:
                    print(f"  警告: 零值占比过高 ({zero_percent:.1f}%)")
                if act_std < 0.01:
                    print(f"  警告: 激活方差过小 ({act_std:.6f})")
                if act_max - act_min < 0.1:
                    print(f"  警告: 激活范围过窄 ({act_max - act_min:.6f})")
...
for iter in range(epochs):
    ...
    for idx in idxs_users:
        ...
        # 使用聚类模型初始化客户端模型
        if current_cluster in net_glob_client_cluster:
            print(f"为客户端 {idx} (聚类 {current_cluster}) 加载聚类模型参数")
            
            # 使用安全加载函数从聚类模型加载参数
            load_model_safely(
                net_glob_client,
                net_glob_client_cluster[current_cluster].state_dict(),
                preserve_classifier=True  # 保留分类器参数
            )
            
            # 更新客户端模型状态
            net_locals_client_idx[idx] = net_glob_client
            
            # 检查客户端模型的BN层
            print(f"\n检查客户端 {idx} 的模型状态...")
            bn_stats, bn_issues = analyze_batchnorm_layers(net_glob_client)
            if bn_issues:
                print(f"客户端模型存在 {len(bn_issues)} 个批标准化层问题，进行修复...")
                fix_batchnorm_statistics(net_glob_client)
                
                # 如果可用，优先使用客户端本地数据
                if hasattr(client, 'ldr_train') and client.ldr_train is not None:
                    # 使用本地数据校准
                    calibration_loader = torch.utils.data.DataLoader(
                        client.ldr_train.dataset, 
                        batch_size=32, 
                        shuffle=True
                    )
                else:
                    # 使用代理数据集校准
                    calibration_dataset = get_cifar10_proxy_dataset(
                        option='class_samples',  # 使用类别样本
                        num_samples=200,         # 较少样本，快速校准
                        seed=300+idx             # 不同客户端使用不同种子
                    )
                    calibration_loader = torch.utils.data.DataLoader(
                        calibration_dataset, 
                        batch_size=32, 
                ...
                # 校准模型
                net_glob_client = calibrate_model_with_data(
                    net_glob_client, 
                    calibration_loader, 
                    device, 
                    num_batches=5
            ...
            print(f"警告: 客户端 {idx} 的聚类 {current_cluster} 模型不存在，使用默认模型")
        
        # 更新全局模型参数用于计算服务器向客户端传输的数据量
        w_glob_client_tier[client_tier[idx]] = net_glob_client.state_dict()

        client = clients[idx]  # 使用已存在的客户端实例
        # # 诊断客户端模型状态
        # client_model = copy.deepcopy(net_glob_client).to(device)
        # print(f"\n检查客户端 {idx} 的模型状态...")
        # bn_stats, bn_issues = analyze_batchnorm_layers(client_model)
        # if bn_issues:
        #     print(f"客户端模型存在 {len(bn_issues)} 个批标准化层问题，进行修复...")
        #     fix_batchnorm_statistics(client_model)
        #     # 使用少量本地数据校准模型
        #     client_model = calibrate_model_with_data(
        #         client_model, 
        #         client.ldr_train, 
        #         device, 
        #         num_batches=5
        ...
        # 定义本地训练的轮数（在原代码中可能使用了其他变量名）
        local_epochs = 5  # 设置一个合适的值，或者从配置中获取

        # 使用修复后的模型进行训练
        [Client_local_model, Client_train_time] = client.local_train(
            net=net_glob_client, 
            local_train_epoch=local_epochs
        )
        # 本地训练5轮
        # local_train_epoch = 5
        # [Client_local_model, Client_train_time] = client.local_train(
    ...
    aggregated_clients_models = {}
    aggregated_clients_models = aggregate_clients_models(
        client_clusters, 
        w_locals_client_idx, 
        num_clusters, 
        whether_local_loss, 
    ...
    # 更新各聚类的全局客户端模型并校准
    for cluster_id in range(num_clusters):
        print(f"\n正在加载并校准聚类 {cluster_id} 的聚合模型")
        if cluster_id not in aggregated_clients_models:
            print(f"警告: 聚类 {cluster_id} 在聚合模型中不存在")
            continue
        ...
        # 步骤2: 检查并诊断批标准化层
        bn_stats, bn_issues = analyze_batchnorm_layers(current_model)
        if bn_issues:
            print(f"聚类 {cluster_id} 模型存在 {len(bn_issues)} 个批标准化层问题")
            # 修复批标准化层
            fix_batchnorm_statistics(current_model)
            
        # 步骤3: 使用代表性数据重新校准模型
        if bn_issues:
            print(f"正在校准聚类 {cluster_id} 模型...")
            # 创建平衡的代理数据集用于校准
            calibration_dataset = get_cifar10_proxy_dataset(
                option='balanced_test',  # 类别平衡的测试集
                num_samples=500,         # 使用500个样本进行校准
                seed=42+cluster_id       # 不同聚类使用不同随机种子，但保持可重复性
            )
            calibration_loader = torch.utils.data.DataLoader(
            ...
            # 校准BN层
            current_model = calibrate_global_bn(
                current_model,
                calibration_loader,
                device,
                num_batches=10
        ...
        # 更新存储的状态字典
        w_glob_client_cluster[cluster_id] = current_model.state_dict()
        print(f"聚类 {cluster_id} 模型处理完成")
    ...
    # 使用多层次聚合方法聚合全局模型
    w_glob = enhanced_aggregated_fedavg(
        w_locals_tier, 
        w_locals_client, 
        num_tiers, 
        num_users, 
    ...
    print("\n诊断和修复全局聚合模型...")
    # 创建测试数据加载器
    global_eval_dataset = get_cifar10_proxy_dataset(
        option='balanced_test',  # 类别平衡的测试集
        num_samples=2000,        # 使用更多样本进行全局模型诊断
        seed=100                 # 不同的随机种子
    )
    test_loader = torch.utils.data.DataLoader(
        global_eval_dataset, batch_size=32, shuffle=True
    )
    ...
    bn_issues = diagnostic_results['bn_issues']
    if bn_issues:
        print(f"全局模型存在 {len(bn_issues)} 个批标准化层问题，进行校准...")
        # 创建用于校准的代理数据集
        calibration_dataset = get_cifar10_proxy_dataset(
            option='aug_test',      # 使用数据增强的测试集增加多样性
        ...
        # 校准BN层
        net_glob_fed = calibrate_global_bn(
            net_glob_fed,
            calibration_loader,
            device,
            num_batches=20
    ...
    if classifier_issues:
        print(f"检测到分类器问题，重置分类器...")
        net_glob_fed = reset_classifier_weights(net_glob_fed, num_classes=10)

    # 评估全局模型
    enhanced_evaluate_global_model_on_all_test(
        net_glob=net_glob_fed,
        dataset_test=dataset_test,
        device=device,
    ...
    # 输出层之间的分布差异（每3个epoch执行一次详细分析）
    if iter % 3 == 0:
        print("\n执行详细的模型层分析...")
        # 选择几个关键层进行深入分析
        # 特别关注前面几个层、批标准化层和分类器
        key_layers = ["layer1.0.conv1", "layer2.0.bn1", "layer3.0.relu", "classifier"]
